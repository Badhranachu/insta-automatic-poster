...
if not client.user_id:
    # session invalid, try logging in again
...
def download_random_image_under_1mb():
    ...
    try:
        ...
        for photo in photos:
            ...
            if len(img_data) <= 1 * 1024 * 1024:  # Check if size <= 1MB
...
def chat():
    ...
    profile = ai_profile_col.find_one({"name": "Chitti"})

    if profile:
        system_prompt = profile.get("system_prompt", "Yes, chiitti AI is a professional virtual assistant designed to help with various tasks and provide assistance in a wide range of areas such as scheduling, reminders, research, and more. Feel free to ask any questions or let me know how I can assist you today!..it is like he saying about bekola aii.. iwant to be he is bekola ai..")
    else:
        system_prompt = "Hi! I'm chitti..."
    ...
    try:
        ...
        if "choices" not in data:
            return jsonify({"reply": f"❌ API Error: {data.get('error', 'No choices returned.')}"}), 400
...
from instagrapi import Client
@app.route('/pexels-post', methods=['POST'])
def pexels_post():
    filename = download_random_image_under_1mb()
    if filename:
        try:
            caption = "animation"  # use your existing static caption function
            client.photo_upload(path=filename, caption=caption)
            log_event(f"✅ Posted image from Pexels: {filename}")
            return redirect(url_for('dashboard'))
        except Exception as e:
            log_event(f"❌ Failed to post Pexels image: {str(e)}")
            return redirect(url_for('dashboard'))
    else:
        log_event("⚠️ No image was downloaded from Pexels.")
        return redirect(url_for('dashboard'))
...
def view_logs():
    try:
        with open("bot.log", "r", encoding="utf-8") as f:
            lines = f.readlines()
        lines = [line.strip() for line in lines][-50:]  # show last 50 logs
